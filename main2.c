...
int main(){
    ...
    gen_signal(s1, cos, 8, 0, N);
    gen_signal(s2, sin, 16, 0, N);
    gen_signal(s3, cos, 32, 10, N);
    gen_signal(s4, sin, 12, 50, N);
    ...
    while (!WindowShouldClose())
    {
        ...
        draw_signal_norm_area(s1, N, 0, sh/2, sw, sh/2, BLUE);

        draw_spectrum_rects(freq, N/2,   10, 150, sw, sh/3, BLUE, WHITE);
        draw_spectrum_labels(N/2, N/2,   10, 150, sw, sh/3, 1, LIGHTGRAY);
